...
<workflow command="devspec.story">
  ...
  <rules>
    <rule>Bound this command's material-question queue to intake identity: single-item selection, provider retrieval consent, resolved-item confirmation, and the work-item number. Do not ask behavior, acceptance-criteria, scope-boundary, technical-constraint, edge-case, dependency, compatibility, or validation questions. Record what the source already supplies, list every remaining requirement gap in story.md as an open item, and leave those questions to devspec.grooming.</rule>
    <rule>Ask the queued intake questions in this order: single-item selection, retrieval consent, resolved-item confirmation, then the work-item number. The number question runs last because its choices depend on the resolved provider identifier.</rule>
