...
<workflow command="devspec.extract">
  ...
  <rules>
    ...
    <rule>For each observed coding convention that changes implementation or review behavior, capture a concise, concrete local code or test example developers can follow and cite its source location as evidence. Record a relevant anti-pattern when the repository supplies evidence for one.</rule>
