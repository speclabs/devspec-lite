...
Work item: `story → refine → finalize → tasks → implement → review`. Refinement is the default step after intake; skip it only when the intake source itself carried explicit acceptance criteria and story.md lists no open requirement gap. Story intake accepts one manual request or, when an authenticated MCP connector is available, one read-only provider work-item reference.
