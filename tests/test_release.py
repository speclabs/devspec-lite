...
from devspec_lite import __version__
...
class ReleaseMetadataTests(unittest.TestCase):
    def test_pyproject_reads_the_single_cli_version(self) -> None:
        ...
        self.assertIn('version = {attr = "devspec_lite.__version__"}', pyproject)
        self.assertNotIn('\nversion = "', pyproject)
    ...
    def test_release_templates_are_parameterized(self) -> None:
        winget = (ROOT / "packaging" / "winget" / "DevspecLite.installer.yaml").read_text(encoding="utf-8")
        homebrew = (ROOT / "packaging" / "homebrew" / "devspec-lite.rb").read_text(encoding="utf-8")
