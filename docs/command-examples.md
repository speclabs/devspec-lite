...
| `devspec.coding-standards` | The structure is known and implementation conventions need recording, including a team-defined standard for future work. | `/devspec.coding-standards Add custom standard: validate command inputs at the boundary; follow `ArgumentNullException.ThrowIfNull(input)` before accessing input members.` |
